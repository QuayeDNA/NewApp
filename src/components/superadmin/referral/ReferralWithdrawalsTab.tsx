      ...
      <CardBody>
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm sm:text-base font-semibold flex items-center gap-2" style={{ color: "var(--color-text-primary)" }}>
            <FaHistory className="w-4 h-4" style={{ color: "var(--color-amber)" }} /> Withdrawal History
              ...
              {withdrawals.map((w) => (
                ...
                >
                  ...
                  {typeof w.user === "object" && (
                    <div className="flex items-center gap-1.5 mt-1 text-xs" style={{ color: "var(--color-text-secondary)" }}>
                      <FaUser className="w-3 h-3" />
                      <span>{userName(w.user)}</span>
                      {userEmail(w.user) && (
                        <span className="truncate" style={{ color: "var(--color-text-muted)" }}>· {userEmail(w.user)}</span>
                      )}
                    </div>
                  )}
                  ...
                  {withdrawals.map((w) => (
                    <TableRow key={w._id}>
                      <TableCell>
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            {userEmail(w.user) && (
                              <p className="text-xs" style={{ color: "var(--color-text-muted)" }}>{userEmail(w.user)}</p>
                            )}
                          </div>
                        </div>
                      </TableCell>
                      <TableCell className="whitespace-nowrap">
                        ...
                        <p className="text-xs" style={{ color: "var(--color-text-muted)" }}>{formatTime(w.createdAt)}</p>
                      </TableCell>
                      ...
                      <TableCell className="text-sm max-w-xs truncate" style={{ color: "var(--color-text-secondary)" }}>
                        {w.description || w.metadata?.type || "Commission withdrawal"}
                      </TableCell>
                    </TableRow>
                  ))}
