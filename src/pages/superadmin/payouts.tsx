...
function renderPayoutDestination(destination: PayoutRequestItem['destination']) {
  if (!destination) return <span className="text-xs" style={{ color: 'var(--color-text-secondary)' }}>—</span>;
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-4">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold" style={{ color: 'var(--color-text-primary)' }}>Payout Requests</h1>
          <p className="text-sm mt-1" style={{ color: 'var(--color-text-secondary)' }}>Review and process agent payout requests</p>
        </div>
      ...
      <Card>
        <CardBody>
            ...
            <div className="overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {payouts.map(p => (
                    <TableRow key={p._id}>
                      <TableCell className="whitespace-nowrap text-xs">
                        ...
                        <div style={{ color: 'var(--color-text-muted)' }}>{new Date(p.createdAt).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}</div>
                      </TableCell>
                      <TableCell className="min-w-[200px]">
                        {typeof p.user === 'object' ? (
                          <div>
                            <div className="font-medium" style={{ color: 'var(--color-text-primary)' }}>{(p.user as { fullName?: string }).fullName}</div>
                            <div className="text-xs" style={{ color: 'var(--color-text-secondary)' }}>{(p.user as { email?: string }).email}</div>
                          </div>
                        ) : String(p.user)}
                      </TableCell>
                      <TableCell className="font-medium">GH₵ {p.amount.toFixed(2)}</TableCell>
                      <TableCell className="text-xs" style={{ color: 'var(--color-warning)' }}>
                        {p.transferFee != null ? `GH₵ ${p.transferFee.toFixed(2)}` : '—'}
                      </TableCell>
                      <TableCell className="font-medium" style={{ color: 'var(--color-success)' }}>
                      ...
                      <TableCell>
                        <div className="flex gap-2">
                          ...
                          {p.status === 'processing' && (
                            <span className="text-xs" style={{ color: 'var(--color-ink)' }}>Awaiting…</span>
                          )}
                          {!['pending', 'approved', 'processing'].includes(p.status) && (
                            <span className="text-xs" style={{ color: 'var(--color-text-secondary)' }}>{p.paystackTransfer?.transferReference || '—'}</span>
                          )}
                        </div>
                      </TableCell>
                    </TableRow>
                  ...
                  {payouts.length === 0 && (
                    <TableRow>
                      <TableCell colSpan={8} className="text-center text-sm py-8" style={{ color: 'var(--color-text-secondary)' }}>No payouts found</TableCell>
                    </TableRow>
                  )}
                </TableBody>
              </Table>
          ...
          <div className="mt-4 flex items-center justify-between">
            <div className="text-sm" style={{ color: 'var(--color-text-secondary)' }}>Showing {payouts.length} of {total} results</div>
