...
export const Hero = forwardRef<HTMLElement, HeroProps>(
  ({ 
    ...
    const backgroundClasses = {
      ...
      solid: 'bg-[var(--color-surface-alt)]',
      image: backgroundImage ? `bg-cover bg-center bg-no-repeat` : 'bg-[var(--color-surface-alt)]',
    };
...
export const HeroTitle = forwardRef<HTMLHeadingElement, HeroTitleProps>(
  ({ children, as: Component = 'h1', size = 'lg', className = '', ...props }, ref) => {
    ...
    return (
      <Component
        ref={ref}
        className={`font-bold text-[var(--color-text-primary)] leading-tight ${sizeClasses[size]} ${className}`}
        {...props}
      >
        {children}
      </Component>
...
export const HeroSubtitle = forwardRef<HTMLParagraphElement, HeroSubtitleProps>(
  ({ children, size = 'md', className = '', ...props }, ref) => {
    ...
    return (
      <p
        ref={ref}
        className={`text-[var(--color-text-secondary)] leading-relaxed ${sizeClasses[size]} ${className}`}
        {...props}
      >
        {children}
      </p>
